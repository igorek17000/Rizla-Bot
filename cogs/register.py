...
class Register(commands.Cog):
    ...
    async def register(self, ctx):
        ...
        embed.add_field(name='-register user <nation_id>',
                        value='This command allow to register yourself in the bot database.', inline=True)
        embed.add_field(name='-register admin @user <nation_id>',
                        value='This command allow a admin to register a nation in the database, bypassing the code input.',
                        inline=True)
    ...
    async def user(self, ctx, arg):
        ...
        if key == 'None':
            ...
            requests.post(endpoint, data=values)
     ...
     {{
      ...
      """
                ...
                sql = (
                    '''INSERT INTO data(nation_id, discord_id, nation_name, leader_name, nation_link, last_active, alliance_position, color, beige_turns, vacation_mode_turns, score, soldiers, tanks, aircraft, ships, missiles, nukes, cities, offensive_wars, defensive_wars, avg_infra, alliance_name, alliance_link, nation_flag) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''')
                val = (
                    natid, ctx.message.author.id, natname, leadname, natlink, convtime, allpos, color, beiget, vacmode,
                    score, soldiers, tanks, aircraft, ships, missiles, nukes, cities, offensive_wars, defensive_wars,
                    average, allname, allurl, flag)
     ...
     {{
      ...
      """
            ...
            sql = (
                '''INSERT INTO data(nation_id, discord_id, nation_name, leader_name, nation_link, last_active, alliance_position, color, beige_turns, vacation_mode_turns, score, soldiers, tanks, aircraft, ships, missiles, nukes, cities, offensive_wars, defensive_wars, avg_infra, alliance_name, alliance_link, nation_flag) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''')
            val = (
                natid, member.id, natname, leadname, natlink, convtime, allpos, color, beiget, vacmode, score, soldiers,
                tanks, aircraft, ships, missiles, nukes, cities, offensive_wars, defensive_wars, average, allname,
                allurl,
                flag)
            cur2.execute(sql, val)
            conn2.commit()
