...
class Keystore(commands.Cog):
    ...
    async def keystore(self, ctx):
        ...
        embed.add_field(name='-keystore dnr <ids>',
                        value='Set a list of alliance ids for exclude them from raid targets list.', inline=True)
    ...
    async def available(self, ctx):
        ...
        embed.add_field(name='commands_channel',
                        value='This value determine the channel you want to use for Data commands.', inline=True)
        embed.add_field(name='commands_role', value='This value determine the role able to use Data commands.',
                        inline=True)
        embed.add_field(name='dnr_list',
                        value='This value force the bot to exclude certain alliance ids from the raid output list.',
                        inline=True)
        await ctx.send(embed=embed)
    ...
    async def settings(self, ctx):
        ...
        guildid = str(res[0])
        apikey = str(res1[0])
        channelcommands = str(res2[0])
        commandsrole = str(res3[0])
        output = str(res4[0])
        dnr = output.split()
        if apikey == 'None':
            ...
            embed.add_field(name='Commands Channel', value=f"<#{channelcommands}>", inline=True)
            embed.add_field(name='Commands Role', value=f"<@&{commandsrole}>", inline=True)
