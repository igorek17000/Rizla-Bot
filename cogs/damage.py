...
class damage(commands.Cog):
 ...
 async def damage(self, ctx):
  ...
  embed.add_field(name='-damage missile <alliance_id>', value='Display targets for high infrastructure damage with missiles.', inline=True)
  embed.add_field(name='-damage nuke <alliance_id>', value='Display targets for high infrastructure damage with nukes.', inline=True)
   ...
   await ctx.send('Error parsing the api key, please contact Simons#7609 for solve the problem')
    ...
    for nations in data:
     ...
     for city in nations['cities']:
      ...
      formula = round(min(random.uniform(300, max(350, pop_density*3)),(avg_infra*0.3 + 100),avg_infra),1)
      inflict = avg_infra - formula
      damage = requests.get(f"https://politicsandwar.com/city/estimate_infra_land_cost.php?q1={str(inflict)}&q2={str(avg_infra)}").text
      if aapos != 'APPLICANT' and color != 'beige' and vacmode == 0 and avg_infra >= 1500 and nat_name not in targets and output <= 4:
       output += 1
       targets += f'[{nat_name}]({nat_link}) - Estimated Damage : ${damage}\n'
  if targets == '' :
   await ctx.send('No targets has been found')
  else:
   embed = discord.Embed(title="Missile Damage Targets", description=targets, color=discord.Colour.from_rgb(58, 194, 243))
   await ctx.send(embed=embed)
 @missile.error
   ...
   await ctx.send('Error parsing the api key, please contact Simons#7609 for solve the problem')
    ...
    for nations in data:
     ...
     for city in nations['cities']:
      ...
      avg_infra = round(float(infrastructure / cities))
      pop_density = round(float(population / land))
      formula = round(min(random.uniform(1700, max(2000, pop_density*13.5)),(avg_infra*0.8 + 150),avg_infra),2)
      inflict = avg_infra - formula
      damage = requests.get(f"https://politicsandwar.com/city/estimate_infra_land_cost.php?q1={str(inflict)}&q2={str(avg_infra)}").text
